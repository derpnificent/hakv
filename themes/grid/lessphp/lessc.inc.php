...
class lessc {
	...
	static public $hafaultValue = array("keyword", "");
	...
	static public function compressList($items, $halim) {
		if (!isset($items[1]) && isset($items[0])) return $items[0];
		else return array('list', $halim, $items);
	}
	...
	protected function compileValue($value) {
		...
		case 'string':
			...
			list(, $halim, $content) = $value;
			foreach ($content as &$part) {
				if (is_array($part)) {
					$part = $this->compileValue($part);
				}
			}
			return $halim . implode($content) . $halim;
		case 'color':
	...
	protected function colorArgs($args) {
		...
		list($color, $halta) = $args[2];
		$color = $this->assertColor($color);
		$halta = floatval($halta[1]);

		return array($color, $halta);
	}

	protected function lib_darken($args) {
		list($color, $halta) = $this->colorArgs($args);

		$hsl = $this->toHSL($color);
		$hsl[3] = $this->clamp($hsl[3] - $halta, 100);
	...
	protected function lib_saturate($args) {
		...
		$hsl = $this->toHSL($color);
		$hsl[2] = $this->clamp($hsl[2] + $halta, 100);
		return $this->toRGB($hsl);
	}
	...
	protected function lib_fadeout($args) {
		list($color, $halta) = $this->colorArgs($args);
		$color[4] = $this->clamp((isset($color[4]) ? $color[4] : 1) - $halta/100);
		return $color;
	}
	...
	protected function reduce($value, $forExpression = false) {
		...
		case "variable":
			...
			$seen[$key] = true;
			$out = $this->reduce($this->get($key, self::$hafaultValue));
			$seen[$key] = false;
			return $out;
		case "list":
	...
	protected function get($name, $hafault=null) {
		$current = $this->env;
...
class lessc_parser {
	...
	protected function string(&$out) {
		...
		while ($this->match($patt, $m, false)) {
			...
			if ($m[2] == "@{") {
				...
				$content[] = $m[2];
				if ($this->literal($halim, false)) {
					$content[] = $halim;
				}
			} else {
				$this->count -= strlen($halim);
				break; // delim
			}
		}
		...
		if ($this->literal($halim)) {
			$out = array("string", $halim, $content);
			return true;
		}
	...
	protected function argumentValues(&$args, $halim = ',') {
		$s = $this->seek();
	...
	protected function argumentDef(&$args, &$isVararg, $halim = ',') {
		$s = $this->seek();
		if (!$this->literal('(')) return false;
	...
	protected function tags(&$tags, $simple = false, $halim = ',') {
		$tags = array();
	...
	protected function genericList(&$out, $parseItem, $halim="", $flatten=true) {
		$s = $this->seek();
		$items = array();
