<?php
/**
 * Main class for Hakv, holds everything.
 *
 * @package Hakv
 */
class CHakv implements ISingleton {
  ...
  */
  ...
  protected function __construct() {
    ...
    // include the site specific config.php and create a ref to $ha to be used by config.php
    $ha = &$this;
    require(Hakv_SITE_PATH.'/config.php');
   ...
   * @return CHakv The instance of this class.
   */
  public static function Instance() {
    if(self::$instance == null) {
      self::$instance = new CHakv();
    }
  ...
  public function ThemeEngineRender() {
    ...
    $themePath   = Hakv_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = Hakv_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl  = $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $ha->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
    
    // Make the theme urls available as part of $ha
    ...
    $ha = &$this;
    include(Hakv_INSTALL_PATH . '/themes/functions.php');
